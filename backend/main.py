    ...
    try:
        ...
        for span in all_spans:
            if span["type"] == "block_break":
                ...
                # FIX: Only flush if it looks like a paragraph end (punctuation) 
                # to handle fragmented PDFs where every line/word is a block.
                if paragraph_buffer:
                    stripped = paragraph_buffer.strip()
                    # If it ends in punctuation, we assume it's a paragraph end.
                    # otherwise we assume it's a fragmentation and merge it.
                    if stripped and stripped[-1] in ['.', '!', '?', ':', ';', '"', '”', ')', ']']:
                         current_section["content"].append({"type": "text", "value": paragraph_buffer.strip()})
                         paragraph_buffer = ""
                    else:
                         # Soft merge: just add space
                         if not paragraph_buffer.endswith(" "):
                            paragraph_buffer += " "
                continue
